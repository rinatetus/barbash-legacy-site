  ...
  return (
    <section className="py-16 px-6 overflow-hidden bg-background border-y border-border">
      <div ref={ref} className={`transition-all duration-1000 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
        <p className="text-xs uppercase tracking-[0.25em] text-gold mb-10 font-body text-center">Institutional Excellence</p>
        ...
        <div className="relative">
          <div className="flex gap-6 animate-marquee whitespace-nowrap">
            {[...institutions, ...institutions].map((inst, i) => (
              <span
                key={i}
                className="inline-flex items-center px-6 py-3 rounded-full border border-border text-sm text-muted-foreground font-body tracking-wide flex-shrink-0 hover:border-gold/50 transition-colors"
              >
                {inst}
              </span>
            ))}
